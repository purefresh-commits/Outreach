...
const openWhatsApp = (contact) => {
  ...
  const message =
    "Hi! " + String.fromCodePoint(0x1F44B) + "\n\n" +
    "We are food product manufacturers — avocado products, guacamole, natural juices, exotic fruit purees, smoothies, corn tortillas, coconut milk, frozen fruits & more. All made with 100% natural ingredients, no additives or preservatives.\n\n" +
...
const openSMS = (contact) => {
  ...
  if (!phone) return alert("No phone number available.");
  const message =
    "Hi! We are food manufacturers with some interesting offers that could be a great fit for your kitchen. " +
    "We would love to share more — who would be the best person to connect with?\n\n" +
    "Learn more: https://worldsgarden.eu/en/ - Natalia | worldsgarden.eu";
...
export default function App() {
  ...
  const submitForm = async () => {
    ...
    if (modal.mode === "add") {
      ...
      const id = form.id;
      const { data, error } = await supabase.from("contacts").update(form).eq("id", id).select();
      if (error) showToast("Error: " + error.message, "error");
  ...
  const filtered = contacts.filter(c => {
    ...
    const ms = !search || [c.firstName, c.companyName, c.email, c.city].some(v => v && v.toLowerCase().includes(search.toLowerCase()));
    return mf && ms;
  });
  ...
  return (
    <div style={{ fontFamily: "Georgia, serif", background: "#f5f0e8", minHeight: "100vh" }}>
      ...
      <div style={{ background: "#2d4a1e", display: "flex", padding: "0 20px", gap: "2px" }}>
        {[["tracker", "Contacts"], ["template", "Templates"], ["instructions", "Setup Guide"]].map(([t, l]) => (
      ...
      <div style={{ padding: "16px 20px" }}>
        {activeTab === "tracker" && (
          <div>
            ...
            <div style={{ display: "flex", gap: "8px", marginBottom: "14px", flexWrap: "wrap" }}>
              <input placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)} style={{ flex: 1, minWidth: "160px", border: "1px solid #ddd", borderRadius: "8px", padding: "7px 12px", fontSize: "13px" }} />
              <select value={filter} onChange={e => setFilter(e.target.value)} style={{ border: "1px solid #ddd", borderRadius: "8px", padding: "7px 10px", fontSize: "12px" }}>
                  ...
                  <div key={c.id} style={{ background: "#fff", borderRadius: "12px", padding: "14px 16px", display: "flex", alignItems: "center", gap: "12px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", flexWrap: "wrap" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
                      <select value={c.status || "Not Sent"} onChange={e => updateStatus(c.id, e.target.value)}
                        style={{ border: "none", borderRadius: "12px", padding: "4px 8px", fontSize: "11px", fontWeight: "bold", background: statusColors[c.status] ? statusColors[c.status].bg : "#f3f4f6", color: statusColors[c.status] ? statusColors[c.status].text : "#374151", cursor: "pointer" }}>
                        {Object.keys(statusColors).map(s => <option key={s}>{s}</option>)}
                      </select>
                    </div>
                    <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                      ...
                      <button onClick={() => openEdit(c)} style={{ background: "#f0f7e6", color: "#2d4a1e", border: "none", borderRadius: "8px", padding: "6px 10px", cursor: "pointer", fontSize: "12px" }}>Edit</button>
                      <button onClick={() => setDeleteConfirm(c.id)} style={{ background: "#fee2e2", color: "#991b1b", border: "none", borderRadius: "8px", padding: "6px 10px", cursor: "pointer", fontSize: "12px" }}>Delete</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeTab === "template" && (
          <div style={{ maxWidth: "620px", margin: "0 auto" }}>
            <div style={{ background: "#fff", borderRadius: "12px", padding: "20px", marginBottom: "16px" }}>
              <div style={{ fontWeight: "bold", color: "#2d4a1e", marginBottom: "14px" }}>Email - Initial Outreach</div>
              <div style={{ background: "#f0f7e6", borderRadius: "8px", padding: "10px 14px", fontSize: "13px", color: "#2d4a1e", fontWeight: "bold", marginBottom: "12px" }}>Subject: [First Name], thought this might be relevant</div>
              <div style={{ background: "#f9fafb", borderRadius: "8px", padding: "14px", fontSize: "13px", lineHeight: "1.8", color: "#333", whiteSpace: "pre-line" }}>{"Hello,\n\nWe are food product manufacturers and have an interesting offer that we believe could be a great fit for your company.\n\n* Excellent prices\n* 24-hour delivery across the US\n* No minimum order required\n* FREE samples available to try our products\n\nBrowse our full product catalog here: https://worldsgarden.eu/en/\n\nAre you the right person to discuss a cost-effective food supply solution designed to maintain high quality, reduce waste, and improve margins?\n\nIf not, could you please point me to the appropriate contact to continue this conversation?\n\nThank you in advance for your reply.\n\nNatalia\nSales | worldsgarden.eu"}</div>
            </div>
            <div style={{ background: "#fff", borderRadius: "12px", padding: "20px", marginBottom: "16px" }}>
              <div style={{ fontWeight: "bold", color: "#2d4a1e", marginBottom: "14px" }}>WhatsApp Message</div>
              <div style={{ background: "#f9fafb", borderRadius: "8px", padding: "14px", fontSize: "13px", lineHeight: "1.8", color: "#333", whiteSpace: "pre-line" }}>{"Hi! 👋\n\nWe are food product manufacturers — avocado products, guacamole, natural juices, exotic fruit purees, smoothies, corn tortillas, coconut milk, frozen fruits & more. All made with 100% natural ingredients, no additives or preservatives.\n\nWe currently have a special offer on our Authentic Guacamole for new partners, and we would love to send you FREE samples to try before committing to anything.\n\n24-hour delivery across the US. No minimum order required.\n\nCheck out our full product range here: https://worldsgarden.eu/en/\n\nWho would be the right person to talk to about this?\n\nNatalia Vargas\npurefresh@worldsgarden.eu\nworldsgarden.eu"}</div>
            </div>
            <div style={{ background: "#fff", borderRadius: "12px", padding: "20px" }}>
              <div style={{ fontWeight: "bold", color: "#2d4a1e", marginBottom: "14px" }}>SMS Message</div>
              <div style={{ background: "#f9fafb", borderRadius: "8px", padding: "14px", fontSize: "13px", lineHeight: "1.8", color: "#333", whiteSpace: "pre-line" }}>{"Hi! We are food manufacturers with some interesting offers that could be a great fit for your kitchen. We would love to share more — who would be the best person to connect with?\n\nLearn more: https://worldsgarden.eu/en/ - Natalia | worldsgarden.eu"}</div>
            </div>
          </div>
        )}
        ...
        {activeTab === "instructions" && (
          <div style={{ maxWidth: "580px", margin: "0 auto", background: "#fff", borderRadius: "12px", padding: "24px" }}>
            ...
            {[
              ["1", "Add contacts", "Use + Add Contact or Import CSV to add your prospects."],
              ["2", "Send emails", "Click Email button to open Gmail with message pre-filled. Status auto-updates to Sent."],
              ["3", "WhatsApp and SMS", "Click WA or SMS to send pre-filled messages."],
              ["4", "Track replies", "Update status as replies come in using the dropdown."],
              ["5", "Export", "Click Export CSV to download your list for bulk campaigns."],
            ].map(([s, t, d]) => (
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" }}>
          <div style={{ background: "#fff", borderRadius: "16px", padding: "24px", width: "100%", maxWidth: "500px", maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
              <h3 style={{ color: "#2d4a1e", fontSize: "16px" }}>{modal.mode === "add" ? "Add New Contact" : "Edit Contact"}</h3>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
              {[["firstName", "First Name", "text"], ["companyName", "Company Name", "text"], ["email", "Email", "email"], ["phone", "Phone", "text"], ["city", "City", "text"]].map(([f, l, t]) => (
                <div key={f} style={{ gridColumn: f === "email" || f === "companyName" ? "span 2" : "span 1" }}>
                  <label style={{ display: "block", fontSize: "11px", color: "#666", marginBottom: "4px" }}>{l}</label>
                  <input type={t} value={form[f] || ""} onChange={e => setForm({ ...form, [f]: e.target.value })} style={{ width: "100%", border: "1px solid #e5e7eb", borderRadius: "8px", padding: "8px 12px", fontSize: "13px", boxSizing: "border-box" }} />
                </div>
              ))}
              {[["type", "Type", types], ["source", "Source", sources], ["status", "Status", Object.keys(statusColors)]].map(([f, l, opts]) => (
                <div key={f}>
                  <label style={{ display: "block", fontSize: "11px", color: "#666", marginBottom: "4px" }}>{l}</label>
                  <select value={form[f] || ""} onChange={e => setForm({ ...form, [f]: e.target.value })} style={{ width: "100%", border: "1px solid #e5e7eb", borderRadius: "8px", padding: "8px 10px", fontSize: "13px" }}>
              ...
              {[["followUp1", "Follow Up 1"], ["followUp2", "Follow Up 2"], ["lastContact", "Last Contact"]].map(([f, l]) => (
                <div key={f}>
                  <label style={{ display: "block", fontSize: "11px", color: "#666", marginBottom: "4px" }}>{l}</label>
                  <input type="date" value={form[f] || ""} onChange={e => setForm({ ...form, [f]: e.target.value })} style={{ width: "100%", border: "1px solid #e5e7eb", borderRadius: "8px", padding: "8px 10px", fontSize: "13px" }} />
                </div>
              ))}
              <div style={{ gridColumn: "span 2" }}>
                <label style={{ display: "block", fontSize: "11px", color: "#666", marginBottom: "4px" }}>Notes</label>
                <textarea value={form.notes || ""} onChange={e => setForm({ ...form, notes: e.target.value })} rows={3} style={{ width: "100%", border: "1px solid #e5e7eb", borderRadius: "8px", padding: "8px 12px", fontSize: "13px", resize: "vertical", boxSizing: "border-box" }} />
              </div>
            </div>
            <div style={{ display: "flex", gap: "10px", marginTop: "20px", justifyContent: "flex-end" }}>
              ...
              <button onClick={submitForm} style={{ background: "linear-gradient(135deg,#4a7a2e,#7ab040)", color: "#fff", border: "none", borderRadius: "10px", padding: "10px 24px", cursor: "pointer", fontWeight: "bold" }}>
    ...
    <div style={{ fontFamily: "Georgia, serif", background: "#f5f0e8", minHeight: "100vh" }}>
      ...
      <div style={{ padding: "16px 20px" }}>
        ...
        {activeTab === "template" && (
          <div style={{ maxWidth: "620px", margin: "0 auto" }}>
            <div style={{ background: "#fff", borderRadius: "12px", padding: "20px", marginBottom: "16px" }}>
              <div style={{ fontWeight: "bold", color: "#2d4a1e", marginBottom: "14px" }}>Email 1 - Initial Outreach</div>
              <div style={{ background: "#f0f7e6", borderRadius: "8px", padding: "10px 14px", fontSize: "13px", color: "#2d4a1e", fontWeight: "bold", marginBottom: "12px" }}>Subject: [First Name], thought this might be relevant</div>
              <div style={{ background: "#f9fafb", borderRadius: "8px", padding: "14px", fontSize: "13px", lineHeight: "1.8", color: "#333", whiteSpace: "pre-line" }}>{"Hello,\n\nWe are food product manufacturers and have an interesting offer that we believe could be a great fit for your company.\n\n* Excellent prices\n* 24-hour delivery across Canada\n* No minimum order required\n* FREE samples available\n\nAre you the right person to discuss a cost-effective food supply solution?\n\nThank you in advance for your reply.\n\nNatalia\nSales | worldsgarden.eu"}</div>
            </div>
            <div style={{ background: "#fff", borderRadius: "12px", padding: "20px" }}>
              <div style={{ fontWeight: "bold", color: "#2d4a1e", marginBottom: "14px" }}>Email 2 - Follow Up (Day 3)</div>
              <div style={{ background: "#f0f7e6", borderRadius: "8px", padding: "10px 14px", fontSize: "13px", color: "#2d4a1e", fontWeight: "bold", marginBottom: "12px" }}>Subject: Just checking in</div>
              <div style={{ background: "#f9fafb", borderRadius: "8px", padding: "14px", fontSize: "13px", lineHeight: "1.8", color: "#333", whiteSpace: "pre-line" }}>{"Hi,\n\nI reached out a few days ago and wanted to make sure my email did not get buried!\n\nWe still have a special discount on our Authentic Guacamole and would love to send FREE samples.\n\nWho would be the right person to chat with?\n\nNatalia\npurefresh@worldsgarden.eu\n(832) 217-9616\nworldsgarden.eu"}</div>
            </div>
          </div>
        )}
