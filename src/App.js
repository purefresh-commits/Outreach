...
const openWhatsApp = (contact) => {
  ...
  const message = "Hi! We are food ingredient manufacturers, avocado products, purees, peppers & more. We have recently launched some offers that could be a great fit for you.\n\nWho would be the right person to talk to about this?\n\nNatalia Vargas\npurefresh@worldsgarden.eu\nworldsgarden.eu";
...
const openSMS = (contact) => {
  const phone = contact.phone.replace(/[^0-9]/g, "");
  if (!phone) return alert("No phone number available.");
  const message = "Hi! We manufacture avocado products, purees & peppers. We have some offers that could be a great fit for you. Who is the right person to discuss this? - Natalia | purefresh@worldsgarden.eu | worldsgarden.eu";
